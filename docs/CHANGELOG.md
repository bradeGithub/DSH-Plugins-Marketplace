...
## 未发布 / Unreleased（安全加固批次 / Security hardening batch）

集中安全审计落地：安装管线注入面收口、consent 确认门补全、HTTP 边界加固、WebDAV SSRF 封堵、自更新完整性锚（签名 tag 验证）与发布侧硬门控。/ A concentrated security-audit batch: install-pipeline injection surfaces closed, consent gates completed, HTTP boundary hardening, WebDAV SSRF closed, a self-update integrity anchor (signed-tag verification), and release-side hard gates.

- **README CLI 代执行目标白名单（PR #238，重要）**：README 提取的 `dsh plugin install/add <target>` 代执行目标现在必须过 `isCliInstallTarget` 白名单——仅 npm 包名（`[@scope/]name[@version|dist-tag]`）或 GitHub `owner/repo` 两种形态，shell 元字符与 flag 形态一律拒绝并回退常规市场管线（win32 下经 `cmd.exe /c` 拼接，白名单是命令注入防线）/ README-extracted `dsh plugin install/add <target>` delegated-install targets must now pass the `isCliInstallTarget` allowlist — only npm package names or GitHub `owner/repo` shapes are accepted; shell metacharacters and flag shapes are rejected with fallback to the regular marketplace pipeline (on win32 the command is joined via `cmd.exe /c`, making the allowlist the command-injection boundary)
- **consent 确认门补全（PR #241）**：CLI 代执行与 bundle 注册（会写 profile package.json + pnpm install）此前可被仓库 README/清单单方面触发，现均需用户确认——README 内容不再能单方面驱动第三方命令执行或 profile 变更 / the CLI delegated-install and bundle-registration paths (which write the profile package.json and run pnpm install) previously could be triggered unilaterally by repo contents; both now require user confirmation — a README can no longer single-handedly drive third-party command execution or profile mutation
- **安装环境加固（PR #240）**：`cp` 复制过滤器拒绝符号链接——恶意仓库放指向宿主机文件的 symlink 不再能越界读取/复制宿主文件；`answers` 环境变量值收口为 `String`（对齐 env-edit 惯例）/ the copy filter now rejects symlinks — a malicious repo can no longer read/exfiltrate host files via symlinked paths; `answers` env values are coerced to `String` (matching the env-edit convention)
- **HTTP 边界加固（PR #242）**：`?refresh=1` 强制刷新需要 trusted 请求校验——未认证请求不能再随意触发上游 GitHub 拉取（防限流消耗/资源滥用）；安装互斥竞态错误（code=MUTEX_BUSY）由 routes 统一捕获映射 409，不再冒泡成 500 / `?refresh=1` now requires a trusted request — unauthenticated callers can no longer force upstream GitHub fetches (rate-limit / resource-abuse surface); install-mutex busy errors (code=MUTEX_BUSY) are caught in routes and mapped to 409 instead of bubbling to 500
- **WebDAV 备份 SSRF 封堵（PR #243，重要）**：`isSafeWebdavUrl` 升级为 fail-closed 网段分类（仅放行公网主机）；手动重定向逐跳复核（302 跳到内网地址照样拦）；Basic 凭据不再外发到重定向后的非目标主机 / `isSafeWebdavUrl` is now fail-closed network-segment classification (public hosts only); manual redirects are re-validated hop-by-hop (a 302 into an internal address is still blocked); Basic credentials are no longer sent to redirected non-target hosts
- **CI 供应链与适配器杂项（PR #244）**：registry workflow 的 actions 引用全部 pin 到不可变 commit SHA；adaptor 重定向匹配统一小写归一（`keyOf`）；市场缓存 slug 碰撞时按 remote 归属顺延 `~N`（不再错认归属）；auth 模块删除 localhost 死分支 / all actions in the registry workflow are pinned to immutable commit SHAs; adaptor redirect matching is uniformly case-normalized (`keyOf`); cache slug collisions now defer to `~N` directories by remote ownership instead of misattributing; a dead localhost branch in auth was removed
- **自更新完整性锚（PR #245，核心变更）**：市场本体自更新改为**签名 tag 验证通道**——候选 release tag 经本地 `git cat-file` 取 annotated tag 原文，按 SSHSIG wire 格式解析并以编译期信任根（`lib/allowed-signers.js` 的 `ALLOWED_SIGNERS`，按维护者建模 + `REVOKED_KEYS` 吊销集）做 ed25519 验签；候选 semver 降序逐个验证、首个通过者生效（未签名的高版本 tag 不会阻塞更新）；通过后三重绑定（tag 名↔package.json version↔staging HEAD commit SHA）并按 SHA fetch 到 staging、原子替换 + 失败回滚；check 与 run 均 fail-closed，`DSH_MARKETPLACE_ALLOW_UNSIGNED_UPDATE` 为显式逃生口，`DSH_MARKETPLACE_UPDATE_PRERELEASE` 提供预发布通道 opt-in；`marketplace/self-update.json` 记录 `fromSha/toTag/toSha/signedBy` 取证字段（独立文件，不混入插件记录结构） / marketplace self-update now runs over a signed-tag verification channel: candidate release tags are fetched locally via `git cat-file`, the SSHSIG wire format is parsed, and signatures are verified against the compiled-in trust root (`lib/allowed-signers.js` `ALLOWED_SIGNERS`, modeled per-maintainer, plus the `REVOKED_KEYS` revocation set) with ed25519; candidates are tried newest-first and the first verified one wins (an unsigned newer tag cannot block updates); after a triple binding (tag name ↔ package.json version ↔ staging HEAD SHA) the commit is fetched by SHA into staging for atomic swap with rollback; check and run are both fail-closed, with `DSH_MARKETPLACE_ALLOW_UNSIGNED_UPDATE` as an explicit escape hatch and `DSH_MARKETPLACE_UPDATE_PRERELEASE` as an opt-in prerelease channel; `marketplace/self-update.json` records `fromSha/toTag/toSha/signedBy` forensics (a dedicated file, kept out of the plugin record schema)
- **发布侧硬门控（PR #246）**：新增 `scripts/verify-tag.mjs` 共用验签 CLI；`pre-push` hook 拦截未通过验签的 `v*` tag 推送并拒绝 `v*` tag 删除；`tag-verify.yml` CI 在 tag 推送后服务端复核告警；`install-hooks.sh/ps1` 登记 pre-push / new `scripts/verify-tag.mjs` shared verification CLI; a `pre-push` hook blocks `v*` tag pushes that fail signature verification and rejects `v*` tag deletions; `tag-verify.yml` CI re-verifies pushed tags server-side; `install-hooks.sh/ps1` now install the pre-push hook
- **缓存归属探测改用字面 remote URL（PR #247）**：`cacheRemoteOf` 由 `git remote get-url` 改为 `git config --get remote.origin.url`——前者应用 `url.insteadOf` 重写，拿到重写后地址会导致归属比对失配、续装顺延 `~N` 且 deny 清理只删新目录而残留孤儿缓存（真实宿主 e2e 复现修复）/ `cacheRemoteOf` now reads the literal `remote.origin.url` via `git config --get` instead of `git remote get-url` — the latter applies `url.insteadOf` rewrites, which broke ownership matching, deferred re-installs to `~N` directories, and left orphaned caches after deny-cleanup (reproduced and fixed via real-host e2e)
- **测试基建隔离加固（PR #239）**：integration 层并行竞态消除、真实网络与 root registry 泄漏封堵——测试不再可能误触真实网络或仓库根索引文件 / the integration tier's parallel races were removed and real-network / root-registry leaks closed — tests can no longer accidentally reach the real network or the repo-root index files
- **文档（PR #246、#248）**：`docs/RELEASE.md` 新增签名 tag 发布流程与 §2.1 维护者签名 key 管理规程（per-maintainer 信任根：新维护者自提 PR 追加 `.pub`、经已信 key 签名的 release 激活，吊销走 `REVOKED_KEYS`，`signedBy` 按人归因）；`GIT_HOOKS.md` 登记 pre-push 检查项 / `docs/RELEASE.md` gains the signed-tag release procedure and a §2.1 maintainer signing-key management protocol (per-maintainer trust root: a new maintainer opens their own PR adding `.pub`, activated by a release signed with an already-trusted key, revocation via `REVOKED_KEYS`, `signedBy` attributes per maintainer); `GIT_HOOKS.md` now documents the pre-push gate
- **文档与测试基建守卫同步（PR #249）**：README/STANDARD/RELEASE/TESTING/GIT_HOOKS 等文档对齐本轮安全边界与最新计数；`security-guards` 新增文档漂移守卫（测试文件数/入口数、`DSH_MARKETPLACE_*` env 登记、hook 登记 ↔ 文档一致性的静态断言）与 PR 同步清单模板；修复 linked worktree 的 hook 环境链——三个 hook 改用 `git rev-parse --show-toplevel` 定位当前 worktree（原先按共享 hooks 路径上溯会错测主仓），`check.mjs`/`run.mjs` 启动即删除 git 注入的 `GIT_DIR`/`GIT_INDEX_FILE` 等内部变量（经 env 遗传给测试进程后，fixture 仓库的 git init/config/add/commit 会穿透写进真实仓库）；coverage 门禁恢复 100%（6 处可达缺口补测试 + 2 处真实不可达防御路径显式豁免）/ docs (README/STANDARD/RELEASE/TESTING/GIT_HOOKS) synced to the audit landings and current counts; `security-guards` gains doc-drift contracts (test-file/entry counts, `DSH_MARKETPLACE_*` env registration, hook registration ↔ docs consistency) plus a PR sync-checklist template; the linked-worktree hook chain is fixed — hooks now resolve the current worktree via `git rev-parse --show-toplevel` (the old shared-hooks-path walk-up tested the main checkout instead), and `check.mjs`/`run.mjs` strip git-injected `GIT_DIR`/`GIT_INDEX_FILE`-style internals before spawning children (inherited env let fixture-repo git calls punch through into the real repo); the coverage gate is restored to 100% (tests for 6 reachable gaps + explicit exemptions for 2 genuinely unreachable defensive paths)

## v1.6.0 — 2026-09-09（架构：分层解耦 / Architecture: layered decoupling）
...
- **verify-installability SSRF 白名单防护（PR #217，收编外部 #213）**：`fetchJson` 只允许 `https://api.github.com`，拒绝内网/其他主机（CWE-918）——URL 拼自 `registry.json` 的 `full_name`，外部可被 PR 篡改，恶意条目会让 CI 探测脚本把 GitHub token 带到内网资源。收编 @anupamme（OrbisAI Security）的 #213 并补 verify-probe 白名单拒绝/放行断言 / verify-installability SSRF allowlist hardening (PR #217, adopted from external #213): `fetchJson` now only allows `https://api.github.com`, rejecting internal/other hosts (CWE-918) — URLs are built from `registry.json` `full_name` (externally tamperable via PRs), so a malicious entry could carry the GitHub token to internal resources. Adopted from @anupamme (OrbisAI Security)'s #213 with added verify-probe allowlist reject/allow assertions
- **pre-commit 提交内容感知分级运行（重构优化，无新特性）**：按 staged 改动区域决定 pre-commit 跑哪套检查——docs/生成物改动跳过测试快速门（~1s）、仅测试改动跑 unit 快层（~5s）、命中 lib/scripts 核心或无法判定回退全量快速门（~55s，fail-safe 偏保守）；CI（CHECK_DIFF_BASE/CI=true）与 `--only=tests` 强制全量不降级；`.hooksrc` 新增 `precommitStrategy=auto|full` / pre-commit now tiers its checks by staged changes: docs/generated changes skip the test gate (~1s), tests-only runs the unit fast tier (~5s), and lib/scripts core or undeterminable changes fall back to the full fast gate (~55s, fail-safe); CI (CHECK_DIFF_BASE/CI=true) and --only=tests always run full; .hooksrc gains precommitStrategy=auto|full
- **http 请求工具直接单测（重构优化，无新特性）**：`lib/http/request.js` 此前仅被 security-guards 静态断言与 e2e 间接覆盖，现补 `http-request.test.mjs` 直接锁定安全敏感路径——`readJsonBody` 的 1MB 上限 413 / 非法 JSON 400 / 空体 / UTF-8 跨分片、`readBodyLimited` 的 chunked 流式累计超限 cancel / 无 reader 回退、`responseTooLarge` 的 32MB 边界、`json` 序列化；`readJsonBody`/`json` 移出 coverage 豁免（request.js 3/3→5/5 仍 100%）/ lib/http/request.js now has a direct unit test (http-request.test.mjs) locking the security-sensitive paths — readJsonBody's 1MB 413 / invalid-JSON 400 / empty body / UTF-8 cross-chunk, readBodyLimited's chunked over-limit cancel / no-reader fallback, responseTooLarge's 32MB boundary, and json serialization; readJsonBody/json removed from the coverage exemption (request.js 3/3→5/5, still 100%)
