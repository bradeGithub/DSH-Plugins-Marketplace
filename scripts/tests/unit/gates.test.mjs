// 收录门控测试：fork 真排除 + archived 降权字段 + gone 失效清理。
...
// ---- normalize 补 fork/archived 字段 ----
{
  ...
  check("normalize 记录 fork=true", out.fork, true);
  check("normalize 记录 archived=false", out.archived, false);
}
{
  ...
  check("normalize 记录 fork=false（缺省安全）", out.fork, false);
  check("normalize 记录 archived=true", out.archived, true);
}
{
  ...
  check("缺省 fork=false（不误伤旧响应）", out.fork, false);
  check("缺省 archived=false", out.archived, false);
}

// ---- fetchStarSegment fork 排除（mock fetch 集成）----
{
  const items = [
  ...
  check("收录排除 fork 条目", repos.length, 1);
  check("保留真条目", repos[0]?.full_name, "owner/real");
}

// ---- gone 清理（真删除 → 索引剔除，报告归档保留）----
{
  const repos = [
  ...
  check("gone 条目被剔除", out.map((r) => r.full_name), ["a/live", "c/empty"]);
  check("empty 条目保留（空仓库可能很快有内容）", out.some((r) => r.full_name === "c/empty"), true);
  check("报告外条目保留", out.some((r) => r.full_name === "a/live"), true);
}
{
  const out = applyGoneCleanup([{ full_name: "a/b" }], new Map([["a/b", "cordis-plugin"]]));
  check("无 gone 时原样返回", out.length, 1);
}
{
  const out = applyGoneCleanup([{ full_name: "x/y" }], new Map());
  check("空报告原样返回", out.length, 1);
}

// ---- 生命周期：bundle 字段三态写回 + 强制重抓 ----
...
// 契约：① 重抓时非 bundle 必须 delete ② 带 bundle 标记的条目必须进 todo（强制重抓）。
{
  ...
  check("三态写回：非 bundle → delete r.bundle", /if \(isBundlePackage\(pkg\)\) r\.bundle = true; else delete r\.bundle;/.test(src), true);
  check("bundleSuspect：bundle 标记强制重抓", /const bundleSuspect = \(r\) => includeVersion && r\.bundle === true;/.test(src), true);
  check("todo 含 bundleSuspect", /\|\| highStarSuspect\(r\) \|\| bundleSuspect\(r\)/.test(src), true);
}
