...
const MESSAGES = {
  zh: {
    ...
    "cliUnsafeTarget": "README 的 CLI 安装指令目标不是安全的包名/仓库形态，已跳过代执行：{cmd}（继续常规安装流程）",
    "cliUpdateTo": "检测到已有安装，使用显式版本升级：{target}（npm 最新 {version}）",
    "cliDone": "官方 CLI 安装完成 ✔",
    "qCliHeader": "确认代执行 README 的 CLI 安装指令",
    "qCli": "仓库 {repo} 的 README 提供 CLI 安装指令：{cmd}。市场可代为执行（等价于你在终端手动运行该命令）。该指令由第三方仓库提供，是否代执行？选择「不代执行」将回到常规市场安装流程。",
    "optCliContinueDesc": "信任该仓库，代执行 README 提供的 CLI 安装指令",
    "optCliCancel": "不代执行",
    "optCliCancelDesc": "不运行该命令，改用常规市场安装流程",
    "cliSkipped": "用户选择不代执行 README 的 CLI 安装指令，继续常规市场安装流程。",
    ...
    "bundleGateDetected": "检测到 bundle 形态：{name}——经 profile bundles 层注册，将执行 pnpm install。",
    "qBundleHeader": "确认安装 bundle 依赖包",
    "qBundle": "仓库 {repo} 以 bundle 形态发布（{name}，patch: {patch}）。安装将向当前 profile 写入依赖并执行 pnpm install——依赖（含 git/file 来源）的安装脚本可能随之运行，是否允许？选择「不允许」将取消安装并清理所有痕迹。",
    "optBundleAllowDesc": "信任该仓库，写入 profile 依赖并执行 pnpm install",
    "optBundleDenyDesc": "不写入任何依赖，取消安装并清理痕迹",
    "bundleCancelled": "用户不允许 bundle 依赖安装，安装已取消，已清理全部痕迹。",
    ...
    "webdavBadUrl": "WebDAV 地址不合法或指向受限网段：仅允许 http(s) 绝对地址，http 明文仅限局域网主机，回环/链路本地/保留地址一律拒绝",
  ...
  en: {
    ...
    "cliUnsafeTarget": "README CLI install target is not a safe package/repo form — skipped delegated execution: {cmd} (continuing the regular install flow)",
    ...
    "qCliHeader": "Confirm delegated CLI install command",
    "qCli": "Repo {repo}'s README provides a CLI install command: {cmd}. The marketplace can run it for you (equivalent to running it manually in a terminal). The command comes from a third-party repo — run it? Choosing \"Don't run\" falls back to the regular marketplace install flow.",
    "optCliContinueDesc": "Trust this repo and run the README-provided CLI command",
    "optCliCancel": "Don't run",
    "optCliCancelDesc": "Skip the command and use the regular marketplace install",
    "cliSkipped": "User chose not to run the README CLI command — continuing the regular install flow.",
    ...
    "bundleGateDetected": "Bundle form detected: {name} — registering via the profile bundles layer; pnpm install will run.",
    "qBundleHeader": "Confirm bundle dependency install",
    "qBundle": "Repo {repo} ships as a bundle ({name}, patch: {patch}). Installing writes a dependency into the current profile and runs pnpm install — install scripts of the dependency (incl. git/file sources) may run. Allow? Choosing \"Deny\" cancels the install and cleans up all traces.",
    "optBundleAllowDesc": "Trust this repo — write the profile dependency and run pnpm install",
    "optBundleDenyDesc": "Write nothing — cancel the install and clean up traces",
    "bundleCancelled": "User denied the bundle install — cancelled, all traces cleaned up.",
    ...
    "webdavBadUrl": "Invalid or restricted WebDAV URL: only absolute http(s) URLs are allowed, plain http is limited to LAN hosts, and loopback/link-local/reserved addresses are rejected",
