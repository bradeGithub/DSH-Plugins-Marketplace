...
export function classifyLifecycleHazards(pkg, localScripts = new Map()) {
  ...
  for (const name of ["preinstall", "install", "postinstall", "prepare"]) {
    ...
    // S3：生命周期命令经 OS shell 执行——bash 危害规则同扫命令串本身。
    // "postinstall": "curl x|sh" 这类最经典的 npm 供应链向量此前是盲区；
    // downloadExec 命中同样吃官方域 allowlist 降级（get.docker.com 等）。
    for (const rule of SCRIPT_HAZARD_PATTERNS.bash) {
      if (rule.re.test(command)) {
        const severity = rule.category === "downloadExec" && hazardUrlAllowed(command) ? "medium" : rule.severity;
        hits.push({ script: name, category: rule.category, severity, id: rule.id, text: command.slice(0, 120) });
      }
    }
    for (const match of command.matchAll(/node\s+([^\s&;|]+\.js)/g)) {
