...
function normalizeRegistryRepo(r) {
  return {
    ...
    open_issues_count: Number.isFinite(r.open_issues_count) ? r.open_issues_count : null,
...
/** S3 风险命中明细白名单形状校验：id/category/severity 三字符串齐全才保留，上限 6 条（与构建期 scorecard 对齐）。 */
function normalizeRiskFlags(flags) {
  if (!Array.isArray(flags)) return undefined;
  const out = flags.filter(f => f && typeof f.id === "string" && typeof f.category === "string" && typeof f.severity === "string").slice(0, 6);
...
function normalizeRepo(r, kind = "dsh") {
  ...
  const out = {
    ...
    open_issues_count: Number.isFinite(r.open_issues_count) ? r.open_issues_count : null,
    // S1 信号面：构建期回查 git 历史基线算出的 star 增速（7d/30d）——
    // registry 产物有则透传；搜索兜底/旧索引没有 → null（未知，客户端排序垫底）
    stars_delta_7d: Number.isFinite(r.stars_delta_7d) ? r.stars_delta_7d : null,
    stars_delta_30d: Number.isFinite(r.stars_delta_30d) ? r.stars_delta_30d : null,
    default_branch: r.default_branch ?? "main",
    topics: r.topics ?? [],
    ...
    // S3 收录初筛：构建期风险记分卡三档（safe/caution/risk）+ 命中明细。
    // 未评估/评估失败的条目无该字段（诚实未知，前端不显示徽章）；
    // risk_at 是构建内部账本（staleness 标记），不透传给客户端。
    risk_tier: r.risk_tier === "safe" || r.risk_tier === "caution" || r.risk_tier === "risk" ? r.risk_tier : undefined,
