...
/**
 * 扫描克隆缓存中的 README，查找官方 CLI 直接安装指令（`dsh plugin install <repo>`）。
 ...
 * 仅在指令指向的仓库/包与当前仓库一致时返回（大小写不敏感），README 里其他仓库的示例不提示。
 * 找不到返回 null。供安装流程在日志与完成面板中提示「README 提供了官方 CLI 安装方式」。
 */
async function scanCliInstallHint(cacheDir, repo) {
  const target = String(repo ?? "").toLowerCase();
  const nameOnly = target.split("/")[1] ?? "";
  // 候选目标：仓库全名 / 仓库名 / npm 包名（README 指令常用包名，如 `dsh plugin add dshmarket`）
  const candidates = new Set([target, nameOnly]);
  try {
    const pkg = JSON.parse(await readFile(join(cacheDir, "package.json"), "utf8"));
  ...
  for (const file of files) {
    ...
    // 定位所有 install/add <目标> 出现处，检查其前方是否以 `dsh plugin` 开头（允许中间夹 flags）
    for (const m of text.matchAll(/\b(?:install|add)\s+([^\s`"'<>）)\]，。]+)/gi)) {
      ...
      const raw = String(m[1] ?? "").toLowerCase()
        ...
        .replace(/^git@github\.com:/i, "")
        .replace(/\.git$/i, "");
      if (candidates.has(raw) || raw === `github.com/${target}`) {
        // 从「dsh plugin」开始截取整条指令（到行尾/反引号/引号）
        const start = before.search(/\bdsh\s+plugin\b/i);
        const full = (before.slice(start) + m[0]).split(/[`"'\n]/)[0].trim();
        return full;
      }
    }
  }
    ...
    handler: async (req, res) => {
      ...
      const task = (async () => {
        ...
        try {
          ...
          // README 官方 CLI 安装指令提示：README 里写了 `dsh plugin install <repo>` 时，
          // 日志首屏提示（与市场安装等效，二选一），完成/手动响应里带 cliCommand 供复制。
    ...
    handler: async (req, res) => {
      ...
      const task = (async () => {
        ...
        try {
          ...
          } else if (record.type === "cordis-plugin") {
