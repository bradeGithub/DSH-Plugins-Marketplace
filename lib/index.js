...
async function fetchRegistryRepos(kind = "dsh") {
  for (const source of registrySources(kind)) {
    try {
      ...
      // 审查 S3：responseTooLarge 只校验压缩后 Content-Length——解压后大小同样设限
      // （gunzipSync maxOutputLength），防被攻陷 CDN 的压缩炸弹致同步内存耗尽。
      let text;
      if (source.url.endsWith(".gz")) {
        const buf = Buffer.from(await res.arrayBuffer());
        text = gunzipSync(buf, { maxOutputLength: MAX_RESPONSE_BYTES * 4 }).toString("utf8");
      } else {
        text = await res.text();
      }
      const data = JSON.parse(text);
...
async function runNpm(args, opts) {
  if (process.platform === "win32") {
    ...
    // 经 cmd.exe 包装（与 runPnpm 同款）。
    const cmdLine = ["npm.cmd", ...args].map((a) => (/[^\w@/.\-]/.test(a) ? `"${a}"` : a)).join(" ");
    return await execFileAsync("cmd.exe", ["/d", "/s", "/c", cmdLine], opts);
  }
...
async function doSelfUpdate() {
  ...
  try {
    ...
    if (process.platform === "win32") {
      // dsh 是 .cmd 垫片：execFile 无法直接启动（无条件 spawn EINVAL，issue #46），
      // 走 cmd.exe（与 runPnpm / cli 安装管线同款处理）；路径可能含空格，整行按需引号包裹。
      const cli = (await exists(dshCli)) ? dshCli : "dsh";
      const quote = (a) => (/[^\w@/.\-]/.test(a) ? `"${a}"` : a);
      const cmdLine = [quote(cli), ...dshArgs.map(quote)].join(" ");
      // 超时从 600s 收紧到 180s：官方 CLI 内部 spawn pnpm 在 Windows 上可能因 .cmd 垫片
      // EINVAL 立即失败，但也可能挂起——快速失败进入目录替换回退比让用户干等更合理。
      await execFileAsync("cmd.exe", ["/d", "/s", "/c", cmdLine], { timeout: 180000 });
    } else {
      await execFileAsync("dsh", dshArgs, { timeout: 180000 });
    }
  } catch (error) {
          ...
          if (cliInstall) {
            ...
            try {
              ...
              if (process.platform === "win32") {
                ...
                await execFileAsync("cmd.exe", ["/d", "/s", "/c", cmdLine], { cwd: cacheDir, env: cliEnv, timeout: 180000 });
              } else {
