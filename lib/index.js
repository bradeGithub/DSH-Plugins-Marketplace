 ...
 * 仅在指令指向的仓库与当前仓库一致时返回（大小写不敏感；避免展示 README 里别的仓库的示例）。
 * 找不到返回 null。供安装流程在日志与完成面板中提示「README 提供了官方 CLI 安装方式」。
 */
const CLI_CMD_RE = /\bdsh\s+plugin\s+(?:install|add)\s+([\w.-]+\/[\w.-]+|\w[\w.-]*)/gi;
async function scanCliInstallHint(cacheDir, repo) {
  ...
  for (const file of files) {
    ...
    for (const m of text.matchAll(CLI_CMD_RE)) {
      const raw = String(m[1] ?? "").toLowerCase()
        .replace(/^https?:\/\/github\.com\//i, "")
        .replace(/^git@github\.com:/i, "")
        .replace(/\.git$/i, "");
      if (raw === target || raw === nameOnly) return m[0].trim();
    }
  }
  return null;
}
