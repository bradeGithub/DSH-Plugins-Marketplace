...
async function registerBundlePackage(pkgName, version, env, logLine, lang) {
  ...
  // bundle patch 行引用的依赖包名以 bundle 包自身目录为解析锚（harness include 的
  // baseUrl = patch 文件所在目录）。pnpm 下 profile 根的链接是符号链接：ESM 解析按
  // realpath 走 .pnpm 虚拟目录，子包在 bundle 包同级的 .pnpm/X@ver/node_modules 里——
  // 用 createRequire 从 realpath 后的包目录解析，与 harness 实际解析语义一致。
  let depNames = [];
  try {
    const bundlePkg = JSON.parse(await readFile(join(resolvedPkg, "package.json"), "utf8"));
    depNames = Object.keys({ ...(bundlePkg.dependencies ?? {}), ...(bundlePkg.peerDependencies ?? {}) });
