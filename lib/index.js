 ...
 * 引用子包名，必须能从 profile 根按 Node 逐级向上解析（pnpm 布局 + lockfile 对齐）。
 * 成功返回解析后的包目录；失败抛错（错误信息含可操作提示）。
 */
async function registerBundlePackage(pkgName, version, env, logLine, lang) {
  ...
  logLine(t(lang, "bundleRecorded", { name: pkgName, version }));
  logLine(t(lang, "bundlePnpm"));
  try {
    await runPnpm(["install"], { cwd: PROFILE_WEB_DIR, env, timeout: 600000 });
  } catch (error) {
    throw new Error(t(lang, "bundlePnpmFail", { err: String(error?.message ?? error).slice(0, 400) }));
  }
  const resolvedPkg = join(PROFILE_NM, ...pkgName.split("/"));
  if (!(await exists(join(resolvedPkg, "package.json")))) {
    throw new Error(t(lang, "bundleResolveFail", { name: pkgName }));
  }
  return resolvedPkg;
}
...
const MESSAGES = {
  zh: {
    ...
    "bundleRecorded": "已记录到 profile package.json：dependencies[{name}] = {version} + dsh.profile.bundles。",
    ...
    "bundleResolveFail": "pnpm 安装后仍未在 profile node_modules 解析到 {name}——registry 可能缺少该包（镜像源可换 https://registry.npmjs.org），或改用官方 CLI 的 README 安装指令。",
  ...
  en: {
    ...
    "bundleRecorded": "Recorded in profile package.json: dependencies[{name}] = {version} + dsh.profile.bundles.",
    ...
    "bundleResolveFail": "The package {name} still cannot be resolved from the profile node_modules after pnpm install — the registry may not have it (mirrors can switch to https://registry.npmjs.org), or use the official CLI command from the README.",
            ...
            if (targets.length > 0) {
              ...
              for (const pkgName of targets) {
                if (record.bundle === true) {
                  ...
                  try {
                    await runPnpm(["remove", pkgName], { cwd: PROFILE_WEB_DIR, env: buildFilteredEnv(), timeout: 600000 });
                    removed++;
                  } catch (error) {
                    const manifest = await readProfileManifest();
                    if (manifest) {
  ...
  if (type === "cordis-plugin") {
    ...
    for (const root of scanRoots) {
      ...
      if (isBundlePackage(pkg) && repo !== SELF_UPDATE_REPO) {
        ...
        const resolvedPkg = await registerBundlePackage(pkgName, version, env, logLine, lang);
