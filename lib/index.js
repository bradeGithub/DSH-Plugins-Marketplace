...
async function doSelfUpdate() {
  ...
  // dsh CLI 优先用 %APPDATA%\npm\dsh.cmd（start-dsh.bat 同款路径），缺失时回退 PATH 里的 dsh。
              if (process.platform === "win32") {
                ...
                await execFileAsync("cmd.exe", ["/c", "dsh", ...args], { cwd: cacheDir, env: cliEnv, timeout: 180000, maxBuffer: MAX_EXEC_BUFFER });
              } else {  }
...
async function doSelfUpdateByClone() {
  ...
  try {
    ...
    await execFileAsync("git", ["clone", "--depth", "1", `https://github.com/${SELF_UPDATE_REPO}.git`, staging], { timeout: 300000 });
...
function apply(ctx) {
  ...
  try {
    ...
    if (await exists(dshCli)) {
      ...
      await execFileAsync("cmd.exe", ["/c", dshCli, ...dshArgs], { timeout: 180000, maxBuffer: MAX_EXEC_BUFFER });
    } else if (process.platform === "win32") {
      // Windows 自定义 npm prefix（%APPDATA%\npm\dsh.cmd 不存在）：cmd /c 解析 PATH 里的 dsh
      await execFileAsync("cmd.exe", ["/c", "dsh", ...dshArgs], { timeout: 180000, maxBuffer: MAX_EXEC_BUFFER });
    } else {
      await execFileAsync("dsh", dshArgs, { timeout: 180000, maxBuffer: MAX_EXEC_BUFFER });
    }
  } catch (error) {
    ...
    return await doSelfUpdateByClone();
                ...
                await execFileAsync("dsh", args, { cwd: cacheDir, env: cliEnv, timeout: 180000 });
              }
