...
function apply(ctx) {
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        json(res, 200, { repos: deduped, cached_at: listCaches.dsh.at, total: deduped.length, source: listCaches.dsh.source ?? "registry" });
      } catch (error) {
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        json(res, 200, { repos: deduped, cached_at: listCaches.skills.at, total: deduped.length, filtered: list.length, source: listCaches.skills.source ?? "registry" });
      } catch (error) {
        json(res, 500, { error: t(lang, "listFail", { err: String(error?.message ?? error) }) });
      }
    }
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const repo = adaptorRedirectRepo(repo0) ?? repo0;
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const task = (async () => {
        const log = [];
        const logLine = (line) => log.push(line);
        const record = installedMap.get(repo);
        if (!record) {
