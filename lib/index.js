 ...
 * 且装了一个后另一个也会被包名映射误标「已安装」）。
 * 已安装的仓库优先保留（必须仍可见），否则保留 Star 高者；无 pkg_name 的按 full_name 唯一。
 */
 ...
 * dsh 模式额外做 pkg_name 冲突消解（同名包安装目标互斥）；skills 模式不去重
 * （skills 不进 node_modules，同名包不冲突）。
 */
async function fetchAllRepos(kind = "dsh") {
  ...
  return kind === "dsh" ? dedupeReposByPkgName(collected) : collected;
}
