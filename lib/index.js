...
async function runNpm(args, opts) {
  ...
  const execOpts = { ...opts, maxBuffer: MAX_EXEC_BUFFER };
...
async function detectTypeDetail(cacheDir) {
  ...
  if (await has("package.json") && (await looksLikeDshPlugin(await readPackageJsonObject(cacheDir))) === true)
...
async function installNpmTargetToTemp(target) {
  ...
  try {
    ...
    if (process.platform === "win32") {
      ...
      await execFileAsync("cmd.exe", ["/d", "/s", "/c", "npm", ...args], { cwd: tmp, timeout: 300000 });
    } else {
      await execFileAsync("npm", args, { cwd: tmp, timeout: 300000 });
    }
...
async function doSelfUpdate() {
  ...
  if (await exists(dshCli)) {
    // issue #46：Windows 上 Node 的 execFile 无法直接启动 .cmd 批处理（无 shell 参与，
    // 无条件抛 spawn EINVAL）——经 cmd.exe /c 启动。路径作为独立参数（Node 自动加引号，
    // 兼容含空格路径；不能用 /d /s 修饰符——/s 的引号剥离会把路径引号剥掉导致
    // 「不是内部或外部命令」；与 else 分支的 cmd /c 形态一致）。
    // 非 Windows 该路径不可达（APPDATA 未定义 → join 出 npm/dsh.cmd 不存在 → 分支下移）。
    await execFileAsync("cmd.exe", ["/c", dshCli, ...dshArgs], { timeout: 600000, maxBuffer: MAX_EXEC_BUFFER });
  } else if (process.platform === "win32") {
    // Windows 自定义 npm prefix（%APPDATA%\npm\dsh.cmd 不存在）：cmd /c 解析 PATH 里的 dsh
    await execFileAsync("cmd", ["/c", "dsh", ...dshArgs], { timeout: 600000, maxBuffer: MAX_EXEC_BUFFER });
  } else {
    // Linux/macOS：dsh 是 PATH 中的可执行文件，直接调用——v1.4.11 的 cmd /c 在
    // 非 Windows 必然 spawn ENOENT（cmd 不存在），自更新从引入起即不可用
    await execFileAsync("dsh", dshArgs, { timeout: 600000, maxBuffer: MAX_EXEC_BUFFER });
  }
...
async function fetchNpmLatest(pkgName) {
  // 双源并行取新（P1-3）：npmmirror 同步滞后（常规 ~10 分钟，大包被 80MB 同步限制
  // 卡住会无限滞后）——「镜像优先」会漏报新版本，误导「检测更新」。npmjs.org 永远
  // 是真相源，镜像仅作国内可达性兜底：两源都成功取版本较高者（compareVersions），
  // 单源失败取成功者，全失败返回 null。
  ...
  const versions = results.filter((r) => r.status === "fulfilled" && r.value).map((r) => r.value);
...
function apply(ctx) {
        ...
        const worker = async () => {
          while (cursor < repos.length) {
            ...
            if (cliNpmForm) {
              ...
              if (cliParts.length > 2 || cliParts.some((s) => !/^[a-zA-Z0-9._~-]+$/.test(s) || s === "." || s === ".." || s.startsWith("."))) {
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (parts.length > 2 || parts.some((s) => !/^[a-zA-Z0-9._~-]+$/.test(s) || s === "." || s === ".." || s.startsWith("."))) {
      ...
      const task = (async () => {
        ...
        try {
          ...
          if (cacheReuse) {
            ...
            await execFileAsync("git", ["clone", "--depth", "1", `https://github.com/${repo}.git`, cacheDir], { timeout: 180000, maxBuffer: MAX_EXEC_BUFFER });
            ...
            if (await exists(join(cacheDir, ".gitmodules"))) {
              ...
              await execFileAsync("git", ["-c", "protocol.file.allow=never", "submodule", "update", "--init", "--recursive", "--depth", "1"], { cwd: cacheDir, timeout: 180000, maxBuffer: MAX_EXEC_BUFFER });
          ...
          if (cliInstall) {
            ...
            try {
              ...
              if (process.platform === "win32") {
                ...
                await execFileAsync("cmd.exe", ["/c", "dsh", ...args], { cwd: cacheDir, env: cliEnv, timeout: 600000 });
              } else {
  ...
  if (type === "script") {
    ...
    if (usePs1) {
      ...
      if (process.platform === "win32") {
        ...
        if (!/msys|MINGW/i.test(`${r.stdout ?? ""}${r.stderr ?? ""}`)) shArgs = [wslPosixPath(join(cacheDir, "install.sh"))];
      }
      await execFileAsync("bash", shArgs, { cwd: cacheDir, env, timeout: 600000 });
    } else {
