...
async function doSelfUpdate() {
  ...
  const tmp = join(MARKET_ROOT, `self-update-${Date.now()}`);
  const repoDir = join(tmp, "repo");
  try {
    await mkdir(repoDir, { recursive: true });
    await execFileAsync("git", ["clone", "--depth", "1", `https://github.com/${SELF_UPDATE_REPO}.git`, repoDir], { timeout: 180000 });
    const newPkg = JSON.parse(await readFile(join(repoDir, "package.json"), "utf8"));
    if (typeof newPkg.version !== "string" || compareVersions(installedVersion, newPkg.version) <= 0) {
      return { status: "no-update", installedVersion, latestVersion: newPkg.version ?? latestVersion };
    }
    const destRoot = dirname(MODULE_DIR); // .../node_modules/dsh-plugin-marketplace
    const staging = join(PROFILE_NM, `.dshm-selfupdate-${Date.now()}`);
    const backup = join(PROFILE_NM, `.dshm-selfupdate-backup-${Date.now()}`);
    await mkdir(staging, { recursive: true });
    for (const e of await readdir(repoDir, { withFileTypes: true })) {
      if (e.name === ".git" || e.name === "node_modules") continue;
      await cp(join(repoDir, e.name), join(staging, e.name), { recursive: true });
    }
    // staging 完整性校验：package.json 可读 + 核心入口齐全，缺一不可替换
    const stagedPkg = JSON.parse(await readFile(join(staging, "package.json"), "utf8"));
    if (typeof stagedPkg.version !== "string"
      || !(await exists(join(staging, "lib", "index.js")))
      || !(await exists(join(staging, "lib", "client.js")))) {
      throw new Error("staged package incomplete");
    }
    // 原子替换：旧目录先改名腾位，staging 改名占位；失败把旧目录改回来
    if (await exists(destRoot)) await rename(destRoot, backup);
    try {
      await rename(staging, destRoot);
      // v1.4.10：替换后自我验证——新目录必须可读且版本与 staging 一致，
      // 否则视为替换失败回滚（防「显示成功但文件未变」的静默失败）。
      const swapped = JSON.parse(await readFile(join(destRoot, "package.json"), "utf8"));
      if (typeof swapped.version !== "string" || swapped.version !== stagedPkg.version) {
        throw new Error(`post-swap verification failed: got ${swapped.version}`);
      }
    } catch (error) {
      if (await exists(backup)) await rename(backup, destRoot).catch(() => {});
      throw error;
    }
    await rm(backup, { recursive: true, force: true }).catch(() => {});
    return { status: "done", installedVersion: stagedPkg.version };
  } finally {
      ...
      try {
        ...
        const worker = async () => {
          while (cursor < repos.length) {
            ...
            const slug = slugify(repo.name);
            const owner = slugify(String(repo.full_name).split("/")[0] ?? "");
            // v1.4.10：cli 类型（走 npm 安装）不参与版本检测——npm 发布版本与 GitHub 仓库
            // package.json 版本可能不一致（实测 pi2dsh：npm 0.3.5 vs 仓库 0.10.0），
            // 对比必然失真且「更新」按钮永远消不掉（更新重装后依旧显示）。
            const versionedType = !(record && record.type === "cli");
        ...
        json(res, 200, { repos: deduped, cached_at: listCaches.dsh.at, total: deduped.length, dropped: dropped.length, source: listCaches.dsh.source ?? "registry" });
      } catch (error) {
        ...
        try {
          ...
          if (cliInstall) {
            ...
            try {
              ...
              const args = ["plugin", "--profile", "web", cliInstall.verb === "add" ? "add" : "install", cliInstall.target];
              if (process.platform === "win32") {
      ...
      const task = (async () => {
        ...
        try {
          if (record.type === "skill" || record.type === "agent-preset") {
            ...
            if (record.location) await rm(record.location, { recursive: true, force: true }).catch(() => {});
