...
function registrySources(kind) {
  ...
  // skills.json 已超 1MB（12000+ 仓库），GitHub Contents API 对超 1MB 文件直接 403
  // （"too large"）——skills 模式跳过 api 源，避免每次刷新都白打一个必失败的请求（KIMI 审阅 H4）。
  const sources = [];
  if (kind !== "skills") sources.push({ url: `https://api.github.com/repos/bradeGithub/DSH-Plugins-Marketplace/contents/${file}`, acceptRaw: true });
  sources.push(
...
/**
 * 写磁盘缓存（仅 registry 成功时写入完整索引；search 兜底不落盘，残缺结果永不污染缓存）。
  ...
  webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // 与插件列表一致用并发池（KIMI 审阅 H3）：skills 索引 12000+ 仓库，
        // Promise.all(map) 会瞬间发起上万并发 fs.stat，拖垮 libuv 线程池/触发 EMFILE。
        ...
        json(res, 200, { repos: deduped, cached_at: listCaches.skills.at, total: deduped.length, filtered: list.length, dropped: dropped.length, source: listCaches.skills.source ?? "registry", fp: listFingerprint(deduped) });
      } catch (error) {
        json(res, 500, { error: t(lang, "listFail", { err: String(error?.message ?? error) }) });
      }
    }
