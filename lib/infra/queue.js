...
/**
 * 单飞互斥锁：同一时刻只允许一个任务运行。
 * - run(fn)：执行异步任务，忙时抛 Error（code === "MUTEX_BUSY"）
 * - isBusy()：查询是否正在运行
 * - 任务完成（含失败）后自动释放
 *
 ...
 *
 * busy 错误的 code 标记是调用侧契约：http 路由据此把「isBusy() 预检 →
 * run() 取锁」之间 TOCTOU 竞态的失败映射为 409；fn 内部错误无此 code，
 * 不会被误吞。
 */
function createMutex() {
  ...
  return {
    ...
    async run(fn) {
      if (running !== null) {
        const error = new Error("mutex is busy");
        error.code = "MUTEX_BUSY";
        throw error;
      }
      const task = (async () => fn())();
      running = task;
