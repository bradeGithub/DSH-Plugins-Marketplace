  ...
  factory: (require) => {
    ...
    function injectStyles() {
      var css = [
        ...
        "body[data-dsh-retro]:not([data-ds-dark-theme]) .dshm-dim,body[data-dsh-trading]:not([data-ds-dark-theme]) .dshm-dim,body[data-dsh-xp]:not([data-ds-dark-theme]) .dshm-dim,body[data-dsh-miku]:not([data-ds-dark-theme]) .dshm-dim{color:var(--dsw-alias-label-secondary)!important}",
        // 皮肤包原生 UI 白字浅底修正（更深层问题，页面级）：4 皮肤亮色模式把
        ...
        // 与浅底对比度 >>4.5:1）；仅 4 个问题皮肤 + 仅亮色，深色模式不碰。
        "body[data-dsh-retro]:not([data-ds-dark-theme]),body[data-dsh-trading]:not([data-ds-dark-theme]),body[data-dsh-xp]:not([data-ds-dark-theme]),body[data-dsh-miku]:not([data-ds-dark-theme]){--dsw-alias-label-primary-foreground:var(--dsw-alias-label-primary)!important}"
      ].join("\n");
        ...
        h("div", { style: s.row },
          h("div", { style: { flex: 1, minWidth: 0, marginRight: 12 } },
            h("p", { style: s.name }, repo.name,
              ...
              unverified ? h("span", { className: "dshm-dim", style: s.badge }, t("badgeUnverified")) : null),
            h("p", { className: "dshm-dim", style: s.meta }, repo.full_name + " · " + t("updatedAt", { d: (repo.updated_at || "").slice(0, 10) }) + (repo.license ? " · " + repo.license : "") + " · ",
          ...
          h("div", { style: { flex: "none", display: "flex", flexDirection: "column", gap: 6, alignItems: "stretch" } },
            h("button", {
              className: "dshm-btn" + (done ? " dshm-dim" : " dshm-btn-primary"),
              style: done ? s.btnInstalled : (updateAvailable ? s.btnUpdate : s.btnPrimary),
              disabled: busy || done || props.uninstalling === repo.full_name,
    ...
    function PluginTab(props) {
      ...
      useEffect(function () {
        ...
        fetch("/api/marketplace/list?lang=" + langCurrent, { headers: { "X-DSH-Marketplace": "1" } }).then(function (r) { return r.json(); }).then(function (data) {
          ...
          else { setRepos(data.repos || []); setDataSource(data.source || ""); setDropped(data.dropped || 0); setLastFp(fingerprintOf(data)); }
      ...
      return h("div", null,
        ...
        (dataSource === "cache" || dataSource === "search") ? h("p", { style: s.srcHint }, t(dataSource === "cache" ? "dataSourceCache" : "dataSourceSearch")) : null,
    ...
    function SkillsTab(props) {
      ...
      useEffect(function () {
        ...
        fetch("/api/marketplace/skills?lang=" + langCurrent, { headers: { "X-DSH-Marketplace": "1" } }).then(function (r) { return r.json(); }).then(function (data) {
          if (cancelled) return;
          if (data.error) { setError(data.error); setRepos([]); }
          else { setRepos(data.repos || []); setVisible(SKILL_PAGE_SIZE); setDataSource(data.source || ""); setDropped(data.dropped || 0); setLastFp(fingerprintOf(data)); }
        }).catch(function (err) { if (!cancelled) { setError(String(err)); setRepos([]); } });
        return function () { cancelled = true; };
      }, [props.tick]);
      ...
      return h("div", null,
        ...
        repos ? [
          ...
          list.length === 0 ? h("p", { key: "empty", className: "dshm-dim", style: { fontSize: 13, color: "var(--dsw-alias-label-tertiary)" } }, t("noMatch", { q: query })) : null
        ] : null,
        repos && repos.length === 0 && !error ? h("p", { className: "dshm-dim", style: { fontSize: 13, color: "var(--dsw-alias-label-tertiary)" } }, t("skillsEmpty")) : null
      );
    }
    ...
    function MarketplaceSection() {
      ...
      return h("div", { style: s.page },
        ...
        h("div", { style: s.tabBar },
          h("button", { className: activeTab === "plugins" ? "" : "dshm-dim", style: activeTab === "plugins" ? s.tabActive : s.tabBtn, onClick: function () { setActiveTab("plugins"); } }, t("tabPlugins")),
          h("button", { className: activeTab === "skills" ? "" : "dshm-dim", style: activeTab === "skills" ? s.tabActive : s.tabBtn, onClick: function () { setActiveTab("skills"); } }, t("tabSkills"))
        ),
