window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var DICT_ZH = {
      ...
      badgeRiskSafe: "已扫描",
      badgeRiskSafeTip: "构建期静态扫描未发现风险特征（覆盖安装脚本与 npm 生命周期，非运行时保证）",
      badgeRiskCaution: "注意",
      badgeRiskRisk: "风险",
      badgeRiskFlagsTip: "扫描命中：{flags}",
      riskCat_downloadExec: "下载执行",
      riskCat_obfuscation: "混淆",
      riskCat_dynamicExec: "动态执行",
      riskCat_credRead: "凭证读取",
      riskCat_exfil: "数据外泄",
      riskCat_pathStartup: "持久化驻留",
      riskCat_rcModify: "配置改写",
      riskCat_fileOps: "文件操作",
      typeCli: "官方 CLI",
      typeMap: {
    ...
    var DICT_EN = {
      ...
      badgeRiskSafe: "scanned",
      badgeRiskSafeTip: "No risk patterns found by build-time static scan (covers install scripts and npm lifecycle; not a runtime guarantee)",
      badgeRiskCaution: "caution",
      badgeRiskRisk: "risk",
      badgeRiskFlagsTip: "Scan hits: {flags}",
      riskCat_downloadExec: "download-exec",
      riskCat_obfuscation: "obfuscation",
      riskCat_dynamicExec: "dynamic-exec",
      riskCat_credRead: "credential read",
      riskCat_exfil: "exfiltration",
      riskCat_pathStartup: "persistence",
      riskCat_rcModify: "config modify",
      riskCat_fileOps: "file ops",
      ...
      sortLabel: "Sort",
      sortStars: "Stars",
      sortTrending: "7d trending",
      sortUpdated: "Recently updated",
      sortName: "Name",
      filterVerified: "Verified only",
      filterHideArchived: "Hide archived",
      catAll: "All",
      catOther: "Other",
    ...
    var s = {
      ...
      badgeRiskSafe: { display: "inline-block", padding: "1px 8px", borderRadius: 10, fontSize: 11, border: "1px solid var(--dsw-alias-border-l3)", color: "var(--dsw-alias-label-tertiary)", marginLeft: 8, cursor: "default" },
      badgeRiskCaution: { display: "inline-block", padding: "1px 8px", borderRadius: 10, fontSize: 11, border: "1px solid var(--dsw-alias-state-warn-secondary, #b45309)", color: "var(--dsw-alias-state-warn-primary)", marginLeft: 8, cursor: "default" },
      badgeRiskRisk: { display: "inline-block", padding: "1px 8px", borderRadius: 10, fontSize: 11, border: "1px solid var(--dsw-alias-state-error-secondary, #b91c1c)", color: "var(--dsw-alias-state-error-primary)", marginLeft: 8, cursor: "default" },
    ...
    function PluginTab(props) {
      ...
      // S1 信号面：排序 + 信号过滤状态（stars 默认；verifiedOnly/hideArchived 关闭默认）
      var stateSort = useState("stars"); var sort = stateSort[0]; var setSort = stateSort[1];
      var stateFv = useState(false); var verifiedOnly = stateFv[0]; var setVerifiedOnly = stateFv[1];
      var stateFa = useState(false); var hideArchived = stateFa[0]; var setHideArchived = stateFa[1];
      ...
      // 搜索词/分类/过滤变化时重置分页（搜索在完整数组 filter 后重新分页）
      useEffect(function () { setVisible(PLUGIN_PAGE_SIZE); }, [query, category, sort, verifiedOnly, hideArchived]);
      ...
      return h("div", null,
        ...
        h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center", marginBottom: 8 } },
          h("span", { style: { fontSize: 12, color: "var(--dsw-alias-label-tertiary)" } }, t("sortLabel")),
          h("select", {
            ...
            h("option", { value: "trending" }, t("sortTrending")),
            h("option", { value: "updated" }, t("sortUpdated")),
            h("option", { value: "name" }, t("sortName"))
          ),
          h("label", { style: { display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--dsw-alias-label-secondary)", cursor: "pointer" } },
            h("input", { type: "checkbox", checked: verifiedOnly, onChange: function () { setVerifiedOnly(!verifiedOnly); }, style: { accentColor: "var(--dsw-alias-brand-primary)", cursor: "pointer", margin: 0 } }),
            t("filterVerified")
          ),
          h("label", { style: { display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--dsw-alias-label-secondary)", cursor: "pointer" } },
            h("input", { type: "checkbox", checked: hideArchived, onChange: function () { setHideArchived(!hideArchived); }, style: { accentColor: "var(--dsw-alias-brand-primary)", cursor: "pointer", margin: 0 } }),
            t("filterHideArchived")
          )
        ),
        ...
        repos ? (function () {
          var q = query.trim().toLowerCase();
          var list = filterRepos(repos, query, category, { verifiedOnly: verifiedOnly, hideArchived: hideArchived });
          // S2：有搜索词时相关度优先（字段加权分降序），sort 键做平局回退
          var cmp = repoComparator(sort);
          list.sort(q ? function (a, b) {
            var d = repoMatchScore(b, q) - repoMatchScore(a, q);
          ...
          return [
            h("p", { key: "count", className: "dshm-dim", style: Object.assign({}, s.meta, { margin: "0 0 8px" }) },
              t("countTotal", { n: repos.length }) + ((q || category !== "all" || verifiedOnly || hideArchived) ? t("countMatch", { n: list.length }) : "")),
    ...
    function SkillsTab(props) {
      ...
      // S1 信号面：排序/过滤下推服务端（分页在服务端，参数先于切片，total 计数诚实）
      var stateSort = useState("stars"); var sort = stateSort[0]; var setSort = stateSort[1];
      var stateFv = useState(false); var verifiedOnly = stateFv[0]; var setVerifiedOnly = stateFv[1];
      var stateFa = useState(false); var hideArchived = stateFa[0]; var setHideArchived = stateFa[1];

      function fetchPage(pg, q, refresh, sortKey, fVerified, fHideArchived) {
        var mySeq = ++skillsFetchSeq;
        setLoading(true);
        var sigParams = "&sort=" + encodeURIComponent(sortKey || "stars")
          + (fVerified ? "&verified=1" : "")
          + (fHideArchived ? "&hideArchived=1" : "");
        return fetch("/api/marketplace/skills?lang=" + langCurrent + "&page=" + pg + "&pageSize=" + SKILL_PAGE_SIZE + "&q=" + encodeURIComponent(q || "") + sigParams + (refresh ? "&refresh=1" : ""), { headers: { "X-DSH-Marketplace": "1" } })
      ...
      function doRefresh(force) {
        setRefreshing(true);
        props.showToast(t("refreshing"), true);
        fetchPage(1, query, true, sort, verifiedOnly, hideArchived).finally(function () { setRefreshing(false); });
      }

      // 初始加载 / 外部 tick（安装卸载后刷新第一页）
      useEffect(function () { fetchPage(1, "", false, sort, verifiedOnly, hideArchived); }, [props.tick]);
      // 搜索词/排序/过滤变化 → 重置并加载第 1 页（搜索与信号面均已下推服务端）
      useEffect(function () { fetchPage(1, query, false, sort, verifiedOnly, hideArchived); }, [query, sort, verifiedOnly, hideArchived]);
        ...
        var observer = new IntersectionObserver(function (entries) {
          entries.forEach(function (entry) {
            if (entry.isIntersecting && shouldLoadMore(repos, total, loading)) {
              fetchPage(page + 1, query, false, sort, verifiedOnly, hideArchived);
            }
          });
        }, { rootMargin: "300px" });
        observer.observe(el);
        return function () { observer.disconnect(); };
      }, [repos, query, total, loading, sort, verifiedOnly, hideArchived]);
