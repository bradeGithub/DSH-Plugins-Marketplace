window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        h("div", { style: s.row },
          ...
          h("div", { style: { flex: "none", display: "flex", flexDirection: "column", gap: 6, alignItems: "stretch" } },
            h("button", {
              ...
              style: nonPlugin ? s.btnInstalled : (done ? s.btnInstalled : (updateAvailable ? s.btnUpdate : s.btnPrimary)),
              disabled: busy || done || nonPlugin || props.uninstalling === repo.full_name,
              title: nonPlugin ? t("installDisabledTip") : undefined,
              onClick: function () { props.onInstall(repo.full_name); }
            }, selfBusy ? t("installing") : (updateAvailable ? t("update") : (installed ? t("installed") : (nonPlugin ? t("installDisabled") : t("install"))))),
            installed ? h("button", {
              className: "dshm-btn",
    ...
    function PluginTab(props) {
      ...
      var state8 = useState(false); var refreshing = state8[0]; var setRefreshing = state8[1];      var state9 = useState(PLUGIN_PAGE_SIZE); var visible = state9[0]; var setVisible = state9[1];
        ...
        repos ? (function () {
          ...
          return [
            ...
            shown.map(function (repo) {
              ...
              return h(RepoCard, { key: repo.full_name, repo: repo, installed: repo.installed, updateAvailable: repo.updateAvailable, busy: !!(props.inst && props.inst.phase === "running"), selfBusy: !!(props.inst && props.inst.phase === "running" && props.inst.repo === repo.full_name), uninstalling: props.uninstalling, onEdit: props.onEdit, onUninstall: props.onUninstall, onInstall: function (fullName) { props.setInputValues({}); props.runInstall(fullName, {}, []); } });
            }),
    ...
    function MarketplaceSection() {
      ...
      var tabProps = { inst: inst, runInstall: runInstall, setInputValues: setInputValues, tick: tick, showToast: showToast, uninstalling: uninstallingRepo, onEdit: openEdit, onUninstall: uninstallPlugin };
