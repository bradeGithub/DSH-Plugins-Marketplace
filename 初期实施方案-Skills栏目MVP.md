...
<!-- TOC -->
- [一、本期目标与非目标](#一本期目标与非目标)
  - [目标](#目标)
  - [非目标（本期不做，别手痒）](#非目标本期不做别手痒)
- [二、执行顺序总览](#二执行顺序总览)
- [三、步骤 1：build-registry.mjs 改造（约 1~1.5 小时）](#三步骤-1build-registrymjs-改造约-115-小时)
  - [3.1 模式开关](#31-模式开关)
  - [3.2 多 query 并集拉取](#32-多-query-并集拉取)
  - [3.3 SKILL.md 探测（Trees API，一次调用探四件事）](#33-skillmd-探测trees-api一次调用探四件事)
  - [3.4 增量继承（控额度的命根子）](#34-增量继承控额度的命根子)
  - [3.5 额度护栏](#35-额度护栏)
  - [3.6 输出格式](#36-输出格式)
  - [3.7 额度预算表（贴代码注释里）](#37-额度预算表贴代码注释里)
- [四、步骤 2：本地冷启动（1~2 小时，期间可写前端）](#四步骤-2本地冷启动12-小时期间可写前端)
- [五、步骤 3：CI workflow（10 分钟）](#五步骤-3ci-workflow10-分钟)
- [六、步骤 4：服务端路由（lib/index.js，约 30 分钟）](#六步骤-4服务端路由libindexjs约-30-分钟)
- [七、步骤 5：前端 tab（lib/client.js，约 1.5~2 小时）](#七步骤-5前端-tablibclientjs约-152-小时)
- [八、步骤 6：验收清单](#八步骤-6验收清单)
- [九、上线当天（分发动作，零代码）](#九上线当天分发动作零代码)
- [十、本期明确砍掉的东西（写在这里防自己反悔）](#十本期明确砍掉的东西写在这里防自己反悔)
<!-- /TOC -->
